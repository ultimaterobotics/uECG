...
{
	spi_write_reg8(reg<<1, value, mcp.CS);
}

uint8_t mcp3911_read_reg8(uint8_t reg)
{
	return spi_read_reg8((reg<<1) | 1, mcp.CS);
}

void mcp3911_write_reg16(uint8_t reg, uint16_t value)
{
	spi_write_reg16(reg<<1, value, mcp.CS);
}
uint32_t mcp3911_read_reg24(uint8_t reg)
{
	return spi_read_reg24((reg<<1) | 1, mcp.CS);
}
int16_t mcp3911_read_reg16(uint8_t reg)
{
	return spi_read_reg16((reg<<1) | 1, mcp.CS);
}
...
void mcp_start_clock()
{
	...
	NRF_GPIOTE->CONFIG[1] = (1<<20) | (0b11 << 16) | (board_config.mcp_FCLK << 8) | 0b11; //toggle PIN_NUM<<8 (0) TASK
...
uint8_t mcp_rx_buf[16];

void mcp_rx_cplt()
{
	mcp.cvalue[0] = (mcp_rx_buf[1]<<8) | mcp_rx_buf[2]; //MCP3911_CH0);
	mcp.cvalue[1] = (mcp_rx_buf[3]<<8) | mcp_rx_buf[4];
...
void GPIOTE_IRQHandler(void)
{
	if (NRF_GPIOTE->EVENTS_IN[0] == 1)
	{
		...
		if(!spi_is_busy())
			spi_read_buf((MCP3911_CH0<<1) | 1, 4, mcp_rx_cplt, mcp_rx_buf, mcp.CS);
	}
}
